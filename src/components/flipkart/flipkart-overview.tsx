...
export function FlipkartOverview() {
  const [start, setStart] = useState(() => getDateRange("7d").start);
  const [end,   setEnd]   = useState(() => getDateRange("7d").end);
  const [refreshKey, setRefreshKey] = useState(0);
  const [totals,     setTotals]     = useState<FlipkartTotals | null>(null);
  const [prevTotals, setPrevTotals] = useState<FlipkartTotals | null>(null);
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      const [curRes, prevRes, campRes] = await Promise.all([
        fetch(`/api/flipkart/metrics?start_date=${start}&end_date=${end}`),
        fetch(`/api/flipkart/metrics?start_date=${prev.start}&end_date=${prev.end}`),
        fetch(`/api/flipkart/campaigns?start_date=${start}&end_date=${end}`),
      ]);
      if (cancelled) return;
      const [curData, prevData, campData] = await Promise.all([
        curRes.json(), prevRes.json(), campRes.json(),
      ]);
      if (cancelled) return;
  ...
  return (
    <div className="max-w-300 mx-auto space-y-5">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          <h2 className="text-[20px] font-bold text-[#1C2B33] dark:text-[#ededed]">Flipkart Ads — Overview</h2>
          <p className="text-[13px] text-[#65676B] dark:text-[#888888] mt-0.5">
            Product Listing & Smart ROI Ads performance summary
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4">
        ...
        <MetricCard title="Revenue"     value={totals ? `₹${fmtCurrency(totals.revenue)}`      : "—"} change={totals && prevTotals ? calcChange(totals.revenue,     prevTotals.revenue)     : null} />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <div className="bg-white dark:bg-[#111111] rounded-xl border border-[#E4E6EB] dark:border-[#2a2a2a] p-5">
          <h3 className="text-[14px] font-semibold text-[#1C2B33] dark:text-[#ededed] mb-4">Flipkart KPIs</h3>
          <div className="space-y-3">
            {[
              { label: "Revenue (₹)",     value: totals?.revenue     ? `₹${fmtCurrency(totals.revenue)}`  : "—" },
              ...
              { label: "Impressions",     value: totals?.impressions ? fmt(totals.impressions)            : "—" },
            ].map(({ label, value }) => (
      ...
      <div className="bg-white dark:bg-[#111111] rounded-xl border border-[#E4E6EB] dark:border-[#2a2a2a] overflow-hidden">
        ...
        {loading ? (
          ...
          <div className="p-8 text-center text-[13px] text-[#8A8D91] dark:text-[#616161]">
            No campaigns yet — data will appear once Flipkart Ads go live.
          </div>
        ) : (
            ...
            <table className="w-full min-w-[700px]">
              <thead>
                <tr className="border-b border-[#E4E6EB] dark:border-[#2a2a2a] bg-[#F8F9FA] dark:bg-[#161616]">
                  {["Campaign", "Type", "Status", "Spend", "Clicks", "Orders", "Revenue", "ROAS", "CPC"].map((h) => (
              ...
              <tbody>
                {campaigns.map((c) => (
                  <tr key={c.campaign_id} className="border-b border-[#E4E6EB] dark:border-[#2a2a2a] last:border-0 hover:bg-[#F8F9FA] dark:hover:bg-[#1c1c1c]">
                    ...
                    <td className="px-4 py-3 text-right text-[13px] text-[#1C2B33] dark:text-[#ededed] tabular-nums">{c.revenue > 0 ? `₹${fmtCurrency(c.revenue)}` : "—"}</td>
                    ...
                    <td className="px-4 py-3 text-right text-[13px] text-[#1C2B33] dark:text-[#ededed] tabular-nums">{c.cpc > 0 ? `₹${c.cpc.toFixed(2)}` : "—"}</td>
                  </tr>
                ))}
              </tbody>
            </table>
